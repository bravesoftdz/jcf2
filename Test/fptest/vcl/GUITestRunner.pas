...
type
  ...
  private
    ...
    function  GetPropertyName(const aCaption: string): string;
  ...
  protected
    ...
    procedure SetNodeState(Node: TTreeNode; aEnabled :boolean);
...
procedure TGUITestRunner.SetNodeState(Node: TTreeNode; aEnabled: boolean);
var
  MostSeniorChanged :TTReeNode;
begin
   assert(Node <> nil);

   if (NodeToTest(Node).Enabled <> aEnabled) then
     NodeToTest(Node).Enabled := aEnabled;
...
begin
  ...
  RootNode.data := pointer(FTests.Add(ATest));
...
begin
  ...
  with ErrorMessageRTF do
    begin
      ...
      *)
      Lines.Add(Item.SubItems[3] + Item.Caption + ': '); //ParentPath + Test name
//
     //     Lines.Add(obj.FullTestPath + obj.Caption + ': ' + obj.ThrownExceptionName);
    { location }
    Lines.Add('at ' + Item.SubItems[2]);
    { unfiltered error messag }
    Text := Text + LineEnding + (Item.SubItems[4]);
...
begin
  ...
  if GetTextExtentPoint32(ACanvas.Handle,
                          ...
                          ImageSize) then
  begin
    aWidth  := ImageSize.cx + 60;
    aHeight := ImageSize.cy + 4;
  end;
end;
...
function TGUITestRunner.GetPropertyName(const aCaption: string): string;
var
  TempStr: string;
  PosSpace: Integer;
begin
  TempStr := Trim(aCaption);
